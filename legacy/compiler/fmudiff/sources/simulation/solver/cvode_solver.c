...
int cvode_solver_initial(DATA *data, threadData_t *threadData, SOLVER_INFO *solverInfo, CVODE_SOLVER *cvodeData, int isFMI)
{
  ...
  /* upgrade to SUNDIALS v.6*/
  SUNContext SUNCTX_PLACEHOLDER;
  SUNContext_Create(NULL, &SUNCTX_PLACEHOLDER);
  ...
  cvodeData->y = N_VMake_Serial(cvodeData->N, (realtype *)data->localData[0]->realVars, SUNCTX_PLACEHOLDER);
  ...
  cvodeData->cvode_mem = CVodeCreate(cvodeData->config.lmm, SUNCTX_PLACEHOLDER);
  ...
  cvodeData->absoluteTolerance = N_VMake_Serial(cvodeData->N, abstol_tmp, SUNCTX_PLACEHOLDER);
  ...
  cvodeData->y_linSol = N_VNew_Serial(cvodeData->N, SUNCTX_PLACEHOLDER);
  switch (cvodeData->config.jacobianMethod)
  {
  case INTERNALNUMJAC:
  case COLOREDNUMJAC:
    cvodeData->J = SUNDenseMatrix(cvodeData->N, cvodeData->N, SUNCTX_PLACEHOLDER);
    cvodeData->linSol = SUNLinSol_Dense(cvodeData->y_linSol, cvodeData->J, SUNCTX_PLACEHOLDER);
  ...
  switch (cvodeData->config.iter)
  {
    case CV_ITER_FIXED_POINT:
      cvodeData->y_nonLinSol = N_VNew_Serial(cvodeData->N, SUNCTX_PLACEHOLDER);
      cvodeData->nonLinSol = SUNNonlinSol_FixedPoint(cvodeData->y_nonLinSol, cvodeData->N /* Num acceleration vectors for Anderson's method, m <= dimension*/, SUNCTX_PLACEHOLDER);
